...
class Repeater:
    ...
    def get_similar_marks(self, mark, layer, n):
        '''
        ...
        if layer == 4:
            mark = cv2.resize(mark, (105, 105), interpolation=cv2.INTER_AREA)
        else:
            mark = cv2.resize(mark, (45, 45), interpolation=cv2.INTER_AREA)
        # mark = cv2.resize(mark, (81, 81), interpolation=cv2.INTER_AREA)

        start_time_acts = time.time()
        mark_f = format(mark)
        if layer == 4:
            mark_acts = get_layers_output(self.layers, ['conv4'], mark_f)[0]
        else:
        ...
        # print(mark_acts.shape)
        if layer == 4:
            mark_acts = mark_acts[0, 2, 2, :]
        else:
            mark_acts = mark_acts[0, 0, 0, :]
        # L2 normalization
        ...
        if layer == 4:
            indices = self.ANN.get_nn(target, layer, n)
        else:
            indices = self.ANN.get_nn(target, layer, n)
        ...
        for j, idx in enumerate(indices):
            ...
            if layer == 4:
                top_match = self.imgs4[idx]
            else:
                top_match = self.imgs2[idx]
